#clean up monthly wind data from NOOA
#A series of nc4 files to be processed
...
u_wind <- c()
v_wind <- c()
ag_wind <- data.frame(time_loc$Date)
...
for (a in 1:length(name)){
  ...
  v_wind <- c()
  b = 1
  for (b in 1:length(ncfile)){
    nc_data <- nc_open(ncfile[b])
    temp <- time_loc[time_loc$month == b,  c("lon", "lat")]
    temp <- SpatialPoints(temp)
    lon <- ncvar_get(nc_data, "lon")
    lat <- ncvar_get(nc_data, "lat")
    u50m <- ncvar_get(nc_data, "U50M")
  ...
  ag_wind <- cbind(ag_wind, data.frame(sqrt(u_wind^2+v_wind^2)))
  a = a + 1
}

head(ag_wind)
names(ag_wind) <- c("date", "linkou_wind", "taichung_wind", "hsinta_wind", "talin_wind", "hoping_wind", "mailiao_wind")
airbox_processed <- cbind(airbox_processed, ag_wind[2:7])
names(airbox_processed)
